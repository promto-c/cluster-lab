  ...
  if (currentConfig.algorithm === 'AGGLOMERATIVE') {
    ...
    if (
      ...
      cachedLinkage = {
        dataSignature,
        steps: linkage,
        linkageType: currentConfig.linkage,
        metric: currentConfig.metric,
      };
    }
...
function computeSimplifiedSilhouette(vectors: number[][], labels: number[], k: number, distFn: any): number {
  ...
  for (let i = 0; i < k; i++)
    if (counts[i] > 0) for (let d = 0; d < vectors[0].length; d++) centroids[i][d] /= counts[i];
