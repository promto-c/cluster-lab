...
import {
  ...
  loadModel,
  runInference as runDinoInference,
...
const App: React.FC = () => {
  ...
  const rebuildGlobalPcaSamples = useCallback(
    (reason: string, itemsOverride?: GalleryItem[]) => {
      ...
      if (sampled.length > 0) {
        addLog(
          `Global PCA snapshot updated (${reason}) with ${sampled.length} sampled patches (need >= 32 for stable basis).`,
          'info',
        );
        return;
      }
  ...
  const isModelLoaded =
    ...
    status === 'clustering' ||
    (status === 'idle' &&
      logs.some((l) => l.message.includes('Model loaded successfully') || l.message.includes('Algorithms Configured')));
  ...
  const handleEmbeddingsImported = useCallback(
    (nextImages: GalleryItem[], matchCount: number) => {
  ...
  const handleRunAll = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pendingItems.length; i += CHUNK_SIZE) {
        ...
        setGalleryImages((prev) =>
          prev.map((img) => (chunk.some((c) => c.id === img.id) ? { ...img, status: 'processing' } : img)),
        );
  ...
  useEffect(() => {
    if (
      ...
      setGalleryImages((prev) => prev.map((img) => (img.id === targetId ? { ...img, status: 'processing' } : img)));
  ...
  useEffect(() => {
    if (embeddingsReady && !previousEmbeddingsReadyRef.current && !suppressNextEmbeddingsReadyRebuildRef.current) {
      ...
      <main className="flex-1 flex flex-col min-w-0 min-h-0 h-full relative">
        ...
        <div className="flex-1 p-2 sm:p-3 md:p-6 overflow-y-auto overflow-x-hidden scrollbar-thin">
          ...
          {currentStep === AppStep.VISUALIZE && (
            <ScatterView3D items={galleryImages} selectedId={selectedGalleryId} onSelect={handleGallerySelect} />
          )}
        </div>

        {/* Logs Panel - Collapsible */}
        <div
          className={`relative flex flex-col bg-gray-900 border-t border-gray-800 shrink-0 transition-all duration-300 ease-in-out pb-[max(env(safe-area-inset-bottom),0px)] md:pb-0 ${showLogs ? 'h-48' : 'h-9'}`}
        >
          {!showLogs && logProgress && (
            <div className="absolute left-0 top-0 z-10 h-px w-full bg-gray-800 pointer-events-none">
              <div
                ...
                style={{
                  width: `${Math.max(0, Math.min(100, logProgress.value))}%`,
                }}
              />
            </div>
          )}
          ...
          <div
            className="px-3 sm:px-4 py-2 bg-gray-900 border-b border-gray-800 flex items-center justify-between cursor-pointer hover:bg-gray-800/50"
          ...
          >
            <div className="flex items-center gap-2 text-xs font-mono text-gray-500 uppercase overflow-hidden flex-1 mr-4">
              ...
              {!showLogs && logs.length > 0 && (
                <div className="flex items-center overflow-hidden min-w-0 flex-1">
                  ...
                  <span
                    className={`truncate font-normal normal-case ${
            ...
            <div className="flex items-center gap-4 shrink-0">
              <div className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full ${status === 'ready' ? 'bg-green-500' : status === 'error' ? 'bg-red-500' : status === 'idle' ? 'bg-gray-500' : 'bg-accent-500 animate-pulse'}`}
                ></span>
          ...
          <div className="flex-1 overflow-y-auto p-3 font-mono text-[10px] space-y-1 scrollbar-thin bg-gray-950">
            ...
            {logs
              ...
              .map((log, idx) => (
                <div
                  key={idx}
                  className={`flex gap-2 ${log.type === 'error' ? 'text-red-400' : log.type === 'success' ? 'text-green-400' : 'text-gray-500'}`}
                >
