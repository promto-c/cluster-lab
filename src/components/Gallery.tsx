...
import { Image as ImageIcon, Folder, FileJson, Play, CheckCircle, Loader2, AlertCircle, Download, Trash2, CheckSquare, Square, Power, XCircle, Check, Layers, AlertTriangle, HelpCircle, Focus, MinusSquare, ChevronDown, X, Split, Grid, LayoutGrid, Grip, MoreHorizontal } from 'lucide-react';
    ...
    if (isMobile) {
      return (
        <div className="px-3 py-2 bg-gray-800/50 border-b border-gray-800 space-y-2">
              ...
              {onRunAll && selectedBatch.size === 0 && (
                isProcessing ? (
                  <Button
                    ...
                    variant="danger"
                    icon={XCircle}
                    className={!onStop ? "!bg-gray-800 !text-gray-500 !border-gray-700 !cursor-not-allowed !px-2.5" : "!px-2.5"}
                  >
                    Stop
                  </Button>
                ) : (
                  <Button
                    ...
                    variant="primary"
                    className={processedCount === images.length ? "!bg-gray-800 !text-gray-500 !border-gray-700 !cursor-not-allowed !px-2.5" : "!px-2.5"}
                    icon={Play}
                  >
                    Run
                  </Button>
          ...
          {selectedBatch.size === 0 && headerMeta && (
            <div className="overflow-x-auto scrollbar-thin">{headerMeta}</div>
          )}
          ...
          {(processedCount > 0 || isProcessing) && selectedBatch.size === 0 && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-gray-500 font-mono shrink-0">{processedCount}/{images.length}</span>
            </div>
          )}
