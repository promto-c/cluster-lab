...
const GalleryCard = React.memo(
  ({ item, isSelected, isCurrent, isEnabled, viewMode, density, onClick, showBatchControls }: GalleryCardProps) => {
    ...
    return (
      ...
      >
        ...
        {isSelected && showBatchControls && (
          <div
            className={`absolute z-20 bg-accent-600 text-white rounded shadow-lg animate-in fade-in zoom-in duration-200 border border-accent-400
        ...
        {!isTiny && (
          <div
            className={`absolute pointer-events-none flex gap-1 ${isCompact ? 'top-0.5 right-0.5' : 'top-2 right-2'}`}
          >
            {item.status === 'processing' && (
              <Loader2 className="w-3.5 h-3.5 text-accent-400 animate-spin drop-shadow-md" />
            )}
            {item.status === 'cached' && !isCompact && (
              <CheckCircle className="w-3.5 h-3.5 text-green-400 fill-green-900/80 drop-shadow-md" />
            )}
...
const Gallery: React.FC<GalleryProps> = ({
      ...
      return (
        <div className="mx-auto w-full max-w-xl flex-1 border-2 border-dashed border-gray-800 rounded-xl flex flex-col items-center justify-center text-gray-500 bg-gray-900/30 p-10 transition-colors hover:bg-gray-900/50 hover:border-gray-700">
          ...
          {(onAddFiles || onAddFolder) && (
            <FileFolderPickerActions onAddFiles={onAddFiles} onAddFolder={onAddFolder} className="mt-5" />
          )}
        </div>
      );
    }
  ...
  const Header = () => {
    if (isMobile) {
      return (
        <div className="px-3 py-2 bg-gray-800/50 border-b border-gray-800 space-y-2">
          <div className="flex items-center justify-between gap-2 min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              {onUpdateItems && (
                <button
                  onClick={selectAll}
                  className="group flex items-center justify-center focus:outline-none transition-colors w-5 h-5 shrink-0"
              ...
              {!onUpdateItems && <ImageIcon className="w-4 h-4 text-accent-500 shrink-0" />}
              <span className="text-xs font-semibold text-gray-300 truncate">
                {viewMode === 'grid' ? 'Dataset Overview' : 'Gallery'}
              </span>
              <span className="px-1.5 py-0.5 rounded-full bg-gray-800 text-[10px] text-gray-500 border border-gray-700 shrink-0">
                {images.length}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {onRunAll &&
                ...
                (isProcessing ? (
                  <Button
                    ...
                    icon={XCircle}
                    className={
                      !onStop ? '!bg-gray-800 !text-gray-500 !border-gray-700 !cursor-not-allowed !px-2.5' : '!px-2.5'
                    }
                  >
                    Stop
                  </Button>
                  ...
                  <Button
                    ...
                    className={
                      processedCount === images.length
                        ? '!bg-gray-800 !text-gray-500 !border-gray-700 !cursor-not-allowed !px-2.5'
                        : '!px-2.5'
                    }
                    icon={Play}
                  >
              ...
              {selectedBatch.size === 0 && (
                <button
                  ...
                  className="inline-flex items-center justify-center w-9 h-9 rounded-md border border-gray-700 bg-gray-800 text-gray-300"
                  title="More actions"
          ...
          {selectedBatch.size === 0 && headerMeta && <div className="overflow-x-auto scrollbar-thin">{headerMeta}</div>}

          {(processedCount > 0 || isProcessing) && selectedBatch.size === 0 && (
          ...
          {selectedBatch.size > 0 && (
            <div className="space-y-2 animate-fadeIn">
              <div className="flex items-center justify-between gap-2 bg-accent-500/10 text-accent-400 px-2 py-1 rounded-md border border-accent-500/20">
                <span className="text-xs font-bold">{selectedBatch.size} Selected</span>
                <button
          ...
          {selectedBatch.size === 0 && showMobileMoreMenu && (
            <div className="rounded-lg border border-gray-700 bg-gray-900/80 p-2 space-y-2 animate-fadeIn">
              {viewMode === 'grid' && (
                  ...
                  {(Object.keys(DENSITY_CONFIG) as Density[]).map((d) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex-1 p-1.5 rounded-md transition-all ${density === d ? 'bg-gray-600 text-white shadow-sm' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-700/50'}`}
              ...
              <div className="flex flex-wrap gap-2">
                {onClear && (
                  <Button onClick={onClear} disabled={isProcessing} variant="danger" icon={Trash2}>
                    Clear
                  </Button>
                )}
                {onImportEmbeddings && (
                ...
                {processedCount > 0 && (
                  <>
                    <Button onClick={() => handleExport(false)} variant="default" icon={Download}>
                      Export Lite
                    </Button>
                    <Button onClick={() => handleExport(true)} variant="default" icon={Download}>
                      Export Full
                    </Button>
                  </>
                )}
          ...
          {selectedBatch.size > 0 ? (
            <div className="flex items-center gap-3 animate-fadeIn">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button onClick={() => batchAction('enable')} disabled={isProcessing} variant="success" icon={Power}>
                  Enable
                </Button>

                <Button onClick={() => batchAction('disable')} disabled={isProcessing} variant="default" icon={XCircle}>
                  Disable
                </Button>

                <Button onClick={() => batchAction('delete')} disabled={isProcessing} variant="danger" icon={Trash2}>
                  Delete
                </Button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full bg-gray-800 text-xs text-gray-500 border border-gray-700">
                  {images.length} items
                </span>
                {headerMeta}
              </div>
                ...
                <>
                  ...
                  <div className="absolute top-full right-0 mt-2 w-48 bg-gray-900 border border-gray-700 rounded-lg shadow-xl z-50 py-1 animate-in fade-in zoom-in-95 duration-100">
                    ...
                    >
                      ...
                      <span className="text-[10px] text-gray-500 group-hover:text-gray-400">
                        Includes patches for 3D view. Large.
                      </span>
                    </button>
                  </div>
                </>
  ...
  return (
    <div
      className={`bg-gray-900 border border-gray-800 rounded-xl flex flex-col overflow-hidden shrink-0 transition-all duration-300 ${viewMode === 'grid' ? 'h-full shadow-inner' : 'h-48'}`}
    >
      <Header />
            ...
            {groupedData.sortedKeys.map((key) => {
              ...
              let titleNode: React.ReactNode = (
                <div className="flex items-center gap-2 text-sm font-bold text-gray-300">
                  ...
                  <span className={isNoise ? 'text-red-400' : isUnassigned ? 'text-gray-500' : 'text-gray-200'}>
                    {key}
                  </span>
                </div>
              );
              ...
              const headerClass = isMasonry
                ...
                : isMobile
                  ? 'bg-gray-900/95 backdrop-blur-sm border-b border-gray-800/50 py-2 px-3 flex items-center justify-between shadow-sm group/header hover:bg-gray-800/50 transition-colors'
                ...
                <div key={key} className={groupContainerClass}>
                  <div className={headerClass} data-gallery-header="true">
                    <div className="flex items-center gap-3">
                      ...
                      <div className={isMasonry ? 'scale-90 origin-left' : ''}>{titleNode}</div>
                    </div>
                      ...
                      {onDrillDown && !isNoise && !isUnassigned && (
                        <button
                          ...
                          className={
                            isMasonry
                              ? isMobile
                                ? 'p-1 rounded text-gray-300 bg-gray-800 hover:text-white hover:bg-accent-600 transition-colors'
                              ...
                              : isMobile
                                ? 'flex items-center gap-1 px-2 py-1 bg-gray-800 text-gray-300 rounded-md transition-colors text-[10px] font-bold uppercase border border-gray-700'
                        ...
                        >
                          ...
                          {!isMasonry && <span className={isMobile ? '' : 'hidden sm:inline'}>Drill Down</span>}
                        </button>
                      )}
