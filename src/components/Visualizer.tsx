...
const Visualizer: React.FC<VisualizerProps> = ({
  ...
  return (
    <div className="relative flex flex-col h-full bg-gray-900 rounded-xl overflow-hidden shadow-2xl border border-gray-800 group">
      ...
      {result && !hasPatches && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/80 backdrop-blur border border-red-500/30 p-6 rounded-xl flex flex-col items-center text-center max-w-sm pointer-events-none">
          ...
          <p className="text-sm text-gray-400">
            This embedding does not contain patch-level data. This usually happens with imported embeddings that were
            exported without patches to save space.
          </p>
      ...
      {result && hasPatches && (
        <div
          className={`absolute top-2 md:top-4 left-2 right-2 md:left-auto md:right-4 md:w-72 bg-gray-900/90 backdrop-blur-md border border-gray-700 rounded-lg shadow-xl transition-all duration-300 z-30 ${showControls ? 'translate-y-0 md:translate-x-0 opacity-100' : 'translate-y-2 md:translate-y-0 md:translate-x-[110%] opacity-0 pointer-events-none'}`}
        >
          ...
          <div className="p-4 space-y-5 text-xs max-h-[65dvh] md:max-h-[60vh] overflow-y-auto">
            ...
            {settings.mode === 'pca' && (
              <div className="space-y-2 animate-fadeIn">
                ...
                {snapshotTimeLabel && (
                  <p className="text-[10px] text-gray-600">Snapshot updated at {snapshotTimeLabel}.</p>
                )}
                ...
                {showPcaInfo && (
                  <div className="rounded-md border border-gray-700 bg-gray-950/70 p-2.5 text-[11px] leading-relaxed text-gray-300 space-y-1">
                    ...
                    <p>
                      <span className="text-gray-200 font-semibold">3 components:</span> PC1/PC2/PC3 are mapped to RGB.
                    </p>
                    <p>
                      <span className="text-gray-200 font-semibold">1 component:</span> PC1 is mapped through the
                      selected colormap.
                    </p>
                  </div>
                )}
            ...
            <SliderField
              label={
                <span className="flex items-center gap-1">
                  <Grid className="w-3 h-3" /> Overlay Opacity
                </span>
              }
      ...
      {result && hasPatches && (
        <div className="absolute bottom-0 left-0 right-0 bg-gray-900/90 border-t border-gray-800 px-4 py-2 flex justify-between items-center text-xs text-gray-400 backdrop-blur-sm z-10 pointer-events-none">
          <div className="flex gap-4">
            <span>
              Grid: {result.dimensions.width}x{result.dimensions.height}
            </span>
